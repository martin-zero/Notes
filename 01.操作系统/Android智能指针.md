...
> Android 的智能指针为了解决 Android 中 Native C++ 指针手动管理的内存泄漏问题，与 C++ 原生的智能指针类似，Android 智能指针使用**引用计数**的方式自动管理对象的生命周期，离开作用域自动释放内存。Android 中智能指针分为 `sp`（强指针）与 `wp`（弱指针）两类，想要被智能指针管理的类必须继承 `RefBase` 类。sp/wp 是 [[ServiceManager自举原理|Binder 通信]]中引用计数管理的基础设施，贯穿整个 Android IPC 体系。
