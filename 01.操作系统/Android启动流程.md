...
> 从按下电源键到 Launcher 显示，Android 启动经过 **BootROM → Bootloader → Kernel → [[Init 流程分析|Init]] → [[Zygote 流程分析|Zygote]] → [[SystemServer 启动流程分析|SystemServer]] → Launcher**。本文是整个启动链路的导航页。
...
2. **[[Init 流程分析]]** — 用户空间第一个进程（PID 1），解析 init.rc，拉起 [[ServiceManager自举原理|ServiceManager]] 和 [[Zygote 流程分析|Zygote]]
